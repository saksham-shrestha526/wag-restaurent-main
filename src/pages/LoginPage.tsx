...
import { Eye, EyeOff, Loader2, X, ChefHat, Mail, KeyRound, Lock, ArrowLeft } from 'lucide-react';
          ...
          <CardHeader className="text-center pt-10 pb-6">
            <div className="flex justify-center mb-4">
              <div className="h-12 w-12 rounded-xl bg-amber-600 flex items-center justify-center shadow-lg">
                {view === 'auth' ? <ChefHat className="h-7 w-7 text-white" /> : view === 'forgot' ? <Mail className="h-7 w-7 text-white" /> : view === 'verify' ? <KeyRound className="h-7 w-7 text-white" /> : <Lock className="h-7 w-7 text-white" />}
              </div>
            </div>
            <CardTitle className="text-2xl font-bold text-white">{header.title}</CardTitle>
            <CardDescription className="text-sm text-gray-400">{header.desc}</CardDescription>
          </CardHeader>
              ...
              <AnimatePresence mode="wait">
                  ...
                  <motion.div key="auth-fields" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.3 }} className="space-y-5">
                    {!isLogin && (
                      <div className="space-y-2">
                        ...
                        <Input id="name" placeholder="Enter your name" required className="bg-gray-800 border-gray-700 text-white" 
                          value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
                      </div>
                    )}
                    <div className="space-y-2">
                      <Label htmlFor="email" className="text-white">Email</Label>
                      <Input id="email" type="email" placeholder="Your email" required className="bg-gray-800 border-gray-700 text-white" 
                        value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="password" className="text-white">Password</Label>
                      <div className="relative">
                        <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Your password" required className="bg-gray-800 border-gray-700 text-white pr-10" 
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <Checkbox id="remember" checked={rememberMe} onCheckedChange={(checked) => setRememberMe(checked as boolean)} className="border-gray-600" />
                        <label htmlFor="remember" className="text-xs text-gray-400 cursor-pointer">Remember me</label>
                      </div>
                      {isLogin && (
                        <button type="button" onClick={() => setView('forgot')} className="text-xs text-amber-500 hover:underline">
                          Forgot password?
                        </button>
                      )}
                    </div>
                ...
                {view === 'forgot' && (
                  <motion.div key="forgot-fields" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-2">
                    ...
                    <Input id="forgot-email" type="email" placeholder="Your email" required className="bg-gray-800 border-gray-700 text-white" 
                      value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                  </motion.div>
                )}
                {view === 'verify' && (
                  <motion.div key="verify-fields" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="code" type="text" placeholder="XXXXXX" maxLength={6} required className="bg-gray-800 border-gray-700 text-white text-center text-2xl tracking-[0.5em] font-bold h-14" 
                        value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value.replace(/\D/g, '').slice(0, 6) })} />
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <div className="text-gray-400">
                        {timer > 0 ? (
                          <span>Code expires in: <span className="font-mono font-bold text-amber-500">{Math.floor(timer / 60)}:{String(timer % 60).padStart(2, '0')}</span></span>
                        ) : (
                          <span className="text-red-400">Code expired</span>
                        )}
                      </div>
                      <button type="button" onClick={handleResendCode} disabled={!canResend || resendLoading} 
                        className={`text-xs font-medium transition-all ${canResend ? 'text-amber-500 hover:underline' : 'text-gray-500 cursor-not-allowed'}`}>
                        {resendLoading ? <Loader2 className="h-3 w-3 animate-spin inline mr-1" /> : null}Resend code
                      </button>
                    </div>
                  </motion.div>
                )}
                {view === 'reset' && (
                  <motion.div key="reset-fields" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <Input id="new-password" type="password" placeholder="••••••••" required className="bg-gray-800 border-gray-700 text-white" 
                        value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="confirm-password" className="text-white">Confirm Password</Label>
                      <Input id="confirm-password" type="password" placeholder="••••••••" required className="bg-gray-800 border-gray-700 text-white" 
                        value={formData.confirmPassword} onChange={(e) => setFormData({ ...formData, confirmPassword: e.target.value })} />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              <Button type="submit" disabled={loading} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold h-11 text-sm">
                {loading ? <Loader2 className="animate-spin mr-2" /> : null}
                {view === 'auth' ? (isLogin ? 'Sign In' : 'Create Account') : view === 'forgot' ? 'Send Reset Code' : view === 'verify' ? 'Verify Code' : 'Reset Password'}
              </Button>
            </form>
            {view === 'auth' && (
              <div className="mt-6 text-center">
                <button onClick={() => setIsLogin(!isLogin)} className="text-sm text-amber-500 hover:underline transition-all">
