...
// ============ SAFE REPLACEMENT: FORCE NEPALI MENU (16 ITEMS) ============
// This block disables foreign keys, clears old menu and orphaned order_items,
// then inserts the 16 Nepali dishes. It uses a transaction to ensure consistency.
console.log('🔄 Replacing menu with 16 Nepali dishes (safe mode)...');
try {
  db.exec('BEGIN TRANSACTION');
  
  // Temporarily disable foreign key checks to allow deletion of referenced menu items
  db.exec('PRAGMA foreign_keys = OFF');
  
  // Delete all order_items that reference menu_items (avoids orphans)
  ...
  db.exec('COMMIT');
  console.log('✅ Nepali menu seeded successfully (16 items)');
} catch (error) {
  console.error('❌ Failed to replace menu:', error);
  db.exec('ROLLBACK');
