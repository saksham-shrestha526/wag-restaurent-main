...
// ============ SAFE FORCED MENU REPLACEMENT (NO FOREIGN KEY ERROR) ============
console.log('🔄 Replacing menu with 16 Nepali dishes (safe mode)...');
try {
  db.exec('BEGIN TRANSACTION');
  // Temporarily disable foreign keys
  db.exec('PRAGMA foreign_keys = OFF');
  // Delete all order_items (orphans)
  ...
  const insertMany = db.transaction((items: typeof nepaliMenu) => {
    for (const item of items) insertMenu.run(item.name, item.description, item.price, item.category, item.is_veg, item.is_spicy, item.image_url);
  });
